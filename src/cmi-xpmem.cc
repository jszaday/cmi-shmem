...
struct ipc_metadata_ {
  ...
  // TODO ( the cache is NOT SMP safe )
...
// (OpenSHMEM has a mechanism for determining addr range?)
...
void CmiCacheBlock(CmiIpcBlock* block) {
  ...
  if (block->cached == nullptr) {
    auto& cache = meta->cache[block->src];
    block->cached = cache;
    cache = block;
  }
}
